...
206. M108 矩形贴图是"敷衍感"的头号来源：不透明 PNG 直接贴上=硬边色块，与羽化背景格格不入（用户红框三连）。教训：**站点/原型内所有大静态贴图必须透明 PNG**（quality-standard 2.6 立为硬法）；只允许径向羽化的场景晕作 wash。
207. M108 透明角色无接地阴影=贴纸漂浮：拆透明后角色与地面无关系，目视即假。教训：透明角色必配椭圆接地软阴影（::after 径向+blur），暗色加极淡 rim glow 保可读。
208. M108 叠图事故：ident 角隅 vignette 与大人物画同占右栏=两画互叠。教训：**同栏不双画**；ident 类品牌视频要么主角要么整段低透明背景（.10-.17+强羽化），不做半主角半装饰。
...
234. M112 发布波更正 M111 #224「scrollWidth 幻影」真凶：04-assets 视图里一个**半截开标签裸文本**（`</div>button data-act=...>保存</button>`，M111 #227 同型伤漏网）作匿名 flex 项撑出 62px 真溢出；M111 法医"隐藏 stage 归零"隐藏的是该文本节点而非壳 bug。教训：溢出法医第一步应是**移除文本节点/匿名项**再二分，且裸文本=可见垃圾，逐屏并排本该肉眼抓到。壳侧顺带加固：#dc-stage 增 position:relative+overflow:hidden（手机屏本就该裁切）。
235. M112 发布走标准车 publish.mjs：门前置（interact/inspect/ui-smoke 三 JSON 齐且**新于 prototype/index.html**）+ privacy 绿 + cover/icon/pages spec 齐才发车；branch+PR+merge 后 workflow 重扫 index。softer 首跑撞 motion-min/empty-band 两标准门：视图零 keyframe+纯渐变底=空带；补 sf-float/sf-pulse idle 动效与 svg 颗粒层（::before opacity≥0.3 才被 empty-band 认作 url() 覆盖）后 48/0。教训：新 run 发布前把标准门全跑一遍，别只跑 demo 期用过的子集。
236. M113 盒子居中≠光学居中：logo-anim webp 透明边距 T17/B9、L6/R12，flex align-items:center 对齐盒子后标记仍偏低偏左。修法=alpha bbox 量光学中心再 CSS 微调（顶栏 translateY(-2px)、底栏 translateX(2px)），并把校验写进 e2e（光学中心差≤2px）。教训：logo 类资产进仓前先 trim 对称或登记 pad 值。
...
243. M114.1 生图白底三连环坑：①seedream 直出"纯白"实为米白+纸纹(237-255)，multiply/screen 下=矩形帷子；②白羽连白底泛洪必漏（淡墨开边即洪入），抠透明不可行；③CSS filter brightness 走 linear-light，sRGB 240 仅到 ~246，推不到 255。正解=sRGB 白点曲线后处理（墨段不动、底环归 255，sharp raw+LUT）+ 提示词要**浓墨**使艺/底色调可分；验证口径=开关差分的"边带 Δ vs 艺核 Δ"（边带≈0 且艺核>0 才算融入且可见）。教训：blend 类装饰资产的底色必须数学纯（255/0），"看起来白"不够。
244. M114.2 白底墨稿装饰资产的唯一稳解=**alpha 墨片**：alpha=255−亮度、色=纯黑，白底像素 alpha 恒 0，矩形在任何主题/blend 下数学不可能；light=multiply 黑墨、dark=容器 invert+screen 白墨，零特殊 CSS 走既有 .sketch 管线。此前 blend 数学（multiply 吃白底）、泛洪抠图（白羽连白底必漏）、linear-light brightness（sRGB 推不到 255）三连败的总教训：别和底色数学作战，把透明做进资产。验证口径=开关差分边带 Δ（应≈0）vs 艺核 Δ（应>0）。
245. M114.2 iframe 嵌入的 load 事件不证明内层 JS 存活（旧 WebView 解析失败也 fire load，露灰屏空壳）；正解=壳 boot 尾 postMessage 心跳 dc-ready，宿主 8s 未收即隐藏 iframe 并给 stagefail+全屏打开出口。教训：跨 frame 健康判定必须应用层心跳，DOM 事件只证明文档到位；已发布壳无心跳期=宿主不能早于壳升级启用硬超时（先同步壳再开门）。
246. M114.2 commit message 英文化入规范：钩子（core.hooksPath 仓内 hooks）+ CI subject ASCII 门双保险；规范只写向前规则（CONTRIBUTING/AGENTS），不写存量处置过程。教训：流程约束要落在机器门上（钩子会漏、CI 不会），文档只陈述规则不陈述运动。
247. M114.3 跨页悬空选中+裸选择器回退=隐形炸弹：点击带 data-goto 的控件先 S.selected=data-dc 值再 loadView，而 loadView 内 M86 redraw 先于 S.selected=null → 新页 el() 落空 → 60-canvas 裸 querySelector(data-dc 值) 对 "as/end" 这类非法裸选择器抛 SyntaxError，被 goto catch 弹成"加载失败"。修=清空提前到 innerHTML 后+裸回退 try/catch 守卫。教训：状态清空必须先于任何消费它的重绘；动态值进 querySelector 一律带引号或 try/catch，回退路径是事故高发区。
248. M114.4 cover.mjs hero 优先 capture 真截图对私域应用=隐私泄露面（微信封面露真公众号文章标题/真联系人，飞书露真工作台人名+真桌面）。修=--hero live 从虚构化视图现拍；私域 run 封面必须 live hero。教训：保真与隐私冲突时，封面/展示面用虚构化产物，capture 真截图只进 gitignore 的 run 内部。
249. M115 genie 会话翻车根因=钥匙发现链有洞+门不机械：标准 key 被 M102 纪律人为限视频，生图不看它；宿主配置无 volces provider 时官方 skill 链也空；consent 是文档约束→agent 静默回落免费档或现场索要 key。修=arkPaidKey 解禁 image+video（成本门=consent 每 session 一次+策略层钳制）、genimg/genvideo 机械门 exit 4（need_consent+ask_text）/exit 5（放 key 指南，非对话索要）、ask_text 首句声明"已检测到 key、无需提供"。教训：门禁必须是脚本退出码；申请话术先回答"要不要我给 key"（不要），再谈花钱同意。
250. M115 修 gate 时挖出 M101 潜伏死代码：genimg defer-agent 块引用未定义变量 A/get/P/W/H→任何直跑必崩，cover.mjs spawnSync 吞掉子进程失败→概念封面静默回落 capture 真截图（隐私泄露的共犯根因之一）。教训：回落链上的潜伏崩溃最隐蔽（调用方吞错）；spawnSync 必检 status；给门禁配 --dry-run 烟测路径；seedream-4-5 图像接口 ≥3686400px 下限与 5.0-lite 标准 key 无权限都要进钳制/模型序真值。
