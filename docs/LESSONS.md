...
151. M62 画廊就绪：用户定稿"meta.json 为唯一真源"——官网读 proto 仓 CI 聚合的 index.json，而 index 扫各 app meta.json；但 SPEC 里图标/封面是**兄弟文件**，meta  alone 不够美观。故生成期产**三件套**（meta.json 双语+tags3-6 / icon.png 真图标链 / cover.png 3:2≤300KB 合成），publish 改搬运+合并；proto 仓零改动。教训：**先核对下游契约（SPEC/index/site 字段）再定产物形态，"用户心智正确"与"契约完整"要同时满足**。
152. M62 封面/图标工程化：genimg 免费档必带 pollinations 水印+灰底暗角——水印裁底带、暗角用**逐行背景估计** keying（角落取样会留大块残背景）；封面=category 色板+设备框+首视图真截图的合成（sharp 单 SVG 内嵌 base64），"与应用场景一致"由真截图保证而非插画赌运气。layout-sanity 对装饰性绝对定位溢出（大爪印水印）仍会报 clipped → 统一用 overflow-hidden 包裹层收口（petpark vip 卡第二次踩同坑）。
