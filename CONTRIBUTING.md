...
1. Read `docs/VISION.md` (what this is for), `docs/DECISIONS.md` (ADRs — deviate only by amending an ADR first), `docs/LESSONS.md` (170+ ways we got bitten — see max number in docs/LESSONS.md).
...
- IP/privacy: no third-party brand assets, fonts, screenshots or faces in tracked files — sole exception: the four curated official icons registered in `docs/THIRD-PARTY.md` §Curated official icons (ip-scan REGISTERED pins them); captures stay in gitignored runs with `knowledge/consent.json` + `knowledge/privacy.json`. See `docs/PROVENANCE.md`, `SECURITY.md`.
...
npm test --prefix skills/design-clone/scripts
```
