...
- 生成物不入库：`publish-out/`、`site/data/thumbs/`（CI 部署前 sync-thumbs 生成）；run 产物在仓内 gitignore 的 `design-clone-runs/`；根目录禁放媒体源文件（源素材归档于空间 `archive/`，M86/M90）；二进制资产入仓必须先登记 `qa/ip-scan.mjs` REGISTERED（M98）
