...
`<app>/meta.json` + `<app>/<flavor>/{meta.json,thumb.png,prototype/,PROVENANCE.md}`；
flavor 词表 `^(mobile|tablet|desktop|web)(-(android|ios|ipad|mac|win|linux))?(-(cn|global))?$`；
每 flavor 只存当前版；改 prototype/** 必须 bump meta.version（CI 红防静默覆盖）；
合并自动打 tag `<app>-<flavor>-<version>` + Release（离线 zip）。

## 发布流程
1. 本地门全绿：`interact`（dead=0）、`inspect`（0 fail）、`ui-smoke`（0 fail）、privacy 账本在场。
2. `publish.mjs --dry` 目检 payload（白名单剥离：无 node_modules/export/qa/capture/视频/字体）。
3. 正式 publish → PR（标题 `publish(app/flavor): title`，body 含门摘要+IP 自律+隐私勾选）。
