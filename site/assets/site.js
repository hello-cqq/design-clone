...
(() => {
  ...
  const I18N = {
    en: {
      ...
      gal_search: "search name or tag…", gal_sort_dl: "most downloads", gal_sort_upd: "recently updated", gal_sort_new: "newest", gal_sort_az: "A→Z",
    ...
    zh: {
      ...
      gal_search: "搜索名称或标签…", gal_sort_dl: "最多下载", gal_sort_upd: "最近更新", gal_sort_new: "最新", gal_sort_az: "字母序",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (sf && sfr && frame) {
      // M114.2：load 事件不足以证明壳 JS 存活（旧 WebView 解析失败也 fire load）→ 以壳 dc-ready 心跳为准
      let ready = false, t = null;
      const showFail = () => {
        sf.classList.remove("hidden");
        frame.style.visibility = "hidden";
        const a = document.getElementById("stageopen");
        const j = document.getElementById("jump");
        if (a && j) a.href = j.href;
      };
      const arm = () => { ready = false; sf.classList.add("hidden"); frame.style.visibility = ""; clearTimeout(t); t = setTimeout(() => { if (!ready) showFail(); }, 9000); };
      addEventListener("message", (ev) => { if (ev && ev.data && ev.data.type === "dc-ready") { ready = true; sf.classList.add("hidden"); frame.style.visibility = ""; } });
